...
use unicorn_engine::{
    Arch, Arm64CpuModel, ContextMode, Mode, Permission as UnicornPermission, TlbType, Unicorn,
};
...
impl<'a, T: Clone> Backend<'a, T> {
    ...
    pub fn emu_start(
        ...
        if option_env!("EMU_LOG") == Some("1") {
            debug!(
                "emu_start: begin: {:#x}, until: {:#x}, timeout: {:#x}, count: {}",
                begin, until, timeout, count
            );
        }
    ...
    #[inline]
    pub fn reg_read(&self, reg_id: RegisterARM64) -> anyhow::Result<u64> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
                .reg_read(reg_id)
    ...
    #[inline]
    pub fn reg_read_u32(&self, reg_id: RegisterARM64) -> anyhow::Result<u32> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
                .reg_read_u32(reg_id)
    ...
    #[inline]
    pub fn reg_write(&self, reg_id: RegisterARM64, value: u64) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
    ...
    #[inline]
    pub fn reg_write_i64(&self, reg_id: RegisterARM64, value: i64) -> anyhow::Result<()> {
        match self {
            #[cfg(feature = "unicorn_backend")]
            Backend::Unicorn(unicorn) => unicorn
                .reg_write_i64(reg_id, value)
    ...
    #[inline]
    pub fn reg_write_i32(&self, reg_id: RegisterARM64, value: i32) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
                .reg_write_i32(reg_id, value)
    ...
    #[inline]
    pub fn reg_write_long(&self, reg_id: RegisterARM64, value: &[u8]) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
                .reg_write_long(reg_id, value)
    ...
    #[inline]
    pub fn mem_read_c_string(&self, address: u64) -> anyhow::Result<String> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
                .mem_read_c_string(address)
    ...
    #[inline]
    pub fn mem_write(&self, address: u64, bytes: &[u8]) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
    ...
    #[inline]
    pub fn mem_write_i64(&self, address: u64, value: i64) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
    ...
    #[inline]
    pub fn mem_read_v2<Data>(&self, address: u64) -> anyhow::Result<Data> {
    ...
    #[inline]
    pub fn mem_read(&self, address: u64, buf: &mut [u8]) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
                .mem_read(address, buf)
    ...
    #[inline]
    pub fn mem_read_i64(&self, address: u64) -> anyhow::Result<i64> {
        let mut buf = [0u8; 8];
    ...
    #[inline]
    pub fn mem_read_u64(&self, address: u64) -> anyhow::Result<u64> {
        let mut buf = [0u8; 8];
    ...
    #[inline]
    pub fn mem_read_i32(&self, address: u64) -> anyhow::Result<i32> {
        let mut buf = [0u8; 4];
    ...
    #[inline]
    pub fn mem_read_as_vec(&self, address: u64, size: usize) -> anyhow::Result<Vec<u8>> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            return unicorn
    ...
    #[inline]
    pub fn context_restore(&self, context: &Context) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
    ...
    #[inline]
    pub fn context_alloc(&self) -> anyhow::Result<Context> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            let uc = unicorn
                .context_alloc()
                .map_err(|e| anyhow!("context_alloc failed: {:?}", e))?;
    ...
    #[inline]
    pub fn context_save(&self, context: &mut Context) -> anyhow::Result<()> {
        #[cfg(feature = "unicorn_backend")]
        if let Backend::Unicorn(unicorn) = self {
            if let Context::Unicorn(context) = context {
                return unicorn
    ...
    pub(crate) fn emu_stop(
        ...
        emulator.inner_mut().running.store(false, Ordering::SeqCst);
