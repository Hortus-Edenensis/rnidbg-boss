...
fn FindClass<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if let Some(vm) = emulator.inner_mut().dalvik.as_mut() {
        vm.throw(
            vm.resolve_class("java/lang/NoClassDefFoundError")
                .unwrap()
                .1
                .new_simple_instance(dalvik!(emulator)),
        );
    }
    RET(JNI_NULL)
}
...
fn NewGlobalRef<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if flag != JNI_FLAG_CLASS && flag != JNI_FLAG_REF && flag != JNI_FLAG_OBJECT {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(object = 0x{:X}) => UnsupportedObject",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("NewGlobalRef"),
                object
            );
        }
        return RET(JNI_NULL);
    ...
    if object == 0 {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(object = 0x{:X}) => JNI_OK",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("DeleteLocalRef"),
                object
            );
        }
        return RET(JNI_OK);
...
fn NewObjectV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if method.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => MethodNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("NewObjectV"),
                env!(emulator),
                clz_id,
                method_id
            );
    ...
    let result = dalvik!(emulator)
        .jni
        .as_mut()
        .expect("JNI not register")
        .call_method_v(
            dalvik!(emulator),
            MethodAcc::CONSTRUCTOR | MethodAcc::OBJECT,
            &class,
            method,
            None,
            &mut va_list,
    ...
    if result.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => JNI_NULL",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("NewObjectV"),
                env!(emulator),
                clz_id,
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => 0x{:X}",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("NewObjectV"),
                env!(emulator),
                clz_id,
    ...
    if flag != JNI_FLAG_REF && flag != JNI_FLAG_OBJECT {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(object = 0x{:X}) => UnsupportedObject",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetObjectClass"),
                object_id
            );
        }
        return RET(JNI_NULL);
...
fn GetMethodID<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    let dvm = dalvik!(emulator);
    let mut clz_id = emulator.backend.reg_read(RegisterARM64::X1).unwrap() as i64;
    let method_name = emulator
        .backend
        .mem_read_c_string(emulator.backend.reg_read(RegisterARM64::X2).unwrap())
        .unwrap();
    let signature = emulator
        .backend
        .mem_read_c_string(emulator.backend.reg_read(RegisterARM64::X3).unwrap())
        .unwrap();
    ...
    if clz.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, name = {}, signature = {}) => ClassNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetMethodID"),
                env!(emulator),
                clz_id,
                method_name,
                signature
    ...
    if accept {
        ...
        member_list.push(DvmMember::Method(DvmMethod::new(
            id,
            clz_id,
            method_name,
            signature,
        ...
        dvm.throw(
            dvm.resolve_class("java/lang/NoSuchMethodError")
                .unwrap()
                .1
                .new_simple_instance(dalvik!(emulator)),
        )
...
fn CallObjectMethodV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if instance.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => ObjectNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallObjectMethodV"),
                env!(emulator),
                object_id,
                method_id
            );
    ...
    if result.is_none() {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => JNI_NULL",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallObjectMethodV"),
                env!(emulator),
                object_id,
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => 0x{:x}",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("CallObjectMethodV"),
            env!(emulator),
            object_id,
...
fn build_jni_varargs<T: Clone>(emulator: &AndroidEmulator<T>) -> anyhow::Result<u64> {
    let gp_save = emulator.falloc(8 * 8, true)?;
    let gp_regs = [
        RegisterARM64::X0,
        RegisterARM64::X1,
        RegisterARM64::X2,
    ...
    for (index, reg) in gp_regs.into_iter().enumerate() {
        let value = emulator
            .backend
            .reg_read(reg)
            .map_err(|err| anyhow!("failed to read {:?}: {:?}", reg, err))?;
        gp_save.write_u64_with_offset((index * 8) as u64, value)?;
    }

    let va_list = emulator.falloc(0x20, true)?;
    va_list.write_u64_with_offset(0, 0)?;
    va_list.write_u64_with_offset(8, gp_save.addr + (8 * 8) as u64)?;
    va_list.write_u64_with_offset(16, 0)?;
    va_list.write_i32_with_offset(24, -(5 * 8))?;
    va_list.write_i32_with_offset(28, 0)?;
    Ok(va_list.addr)
}

fn with_synthesized_varargs<T: Clone>(
    emulator: &AndroidEmulator<T>,
    handler: fn(&str, &AndroidEmulator<T>) -> SvcCallResult,
    name: &str,
) -> SvcCallResult {
    let original_x3 = match emulator.backend.reg_read(RegisterARM64::X3) {
        Ok(value) => value,
        Err(err) => return FUCK(anyhow!("failed to read X3: {:?}", err)),
    };
    let va_list = match build_jni_varargs(emulator) {
        Ok(value) => value,
        Err(err) => return FUCK(err),
    };
    if let Err(err) = emulator.backend.reg_write(RegisterARM64::X3, va_list) {
        return FUCK(anyhow!("failed to write synthesized va_list: {:?}", err));
    }
    let ret = handler(name, emulator);
    let _ = emulator.backend.reg_write(RegisterARM64::X3, original_x3);
    ret
}

fn CallObjectMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallObjectMethodV, name)
}
...
fn CallBooleanMethodV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if instance.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => ObjectNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallBooleanMethodV"),
                env!(emulator),
                object_id,
                method_id
            );
    ...
    if result.is_none() {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => {}",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallBooleanMethodV"),
                env!(emulator),
                object_id,
...
fn CallBooleanMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallBooleanMethodV, name)
}
    ...
    if instance.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => ObjectNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallIntMethodV"),
                env!(emulator),
                object_id,
                method_id
            );
...
fn CallIntMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallIntMethodV, name)
}
...
fn CallVoidMethodV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if instance.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => ObjectNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallVoidMethodV"),
                env!(emulator),
                object_id,
                method_id
            );
    ...
    if result.is_none() {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, object = 0x{:x}, method = 0x{:x}) => JNI_OK",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("CallVoidMethodV"),
                env!(emulator),
                object_id,
...
fn CallVoidMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallVoidMethodV, name)
}

fn GetFieldID<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    let dvm = dalvik!(emulator);
    let mut clz_id = emulator.backend.reg_read(RegisterARM64::X1).unwrap() as i64;
    let field_name = emulator
        .backend
        .mem_read_c_string(emulator.backend.reg_read(RegisterARM64::X2).unwrap())
        .unwrap();
    let signature = emulator
    ...
    if accept {
        ...
        member_list.push(DvmMember::Field(DvmField::new(
            id, clz_id, field_name, signature,
        )));
        ...
        dvm.throw(
            dvm.resolve_class("java/lang/NoSuchFieldError")
                .unwrap()
                .1
                .new_simple_instance(dalvik!(emulator)),
        )
...
fn GetObjectField<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            ...
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("GetObjectField"),
            env!(emulator),
            object_id,
            field_id
        );
...
fn SetObjectField<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(env = 0x{:x}, object = 0x{:x}, field = 0x{:x}, value = 0x{:x})",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("SetObjectField"),
            env!(emulator),
            object_id,
            field_id,
            value
    ...
    if clz.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, name = {}, signature = {}) => ClassNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStaticMethodID"),
                env!(emulator),
                clz_id,
                method_name,
                signature
...
fn CallStaticObjectMethodV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if jni::get_flag_id(clz_id) == JNI_FLAG_REF {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => ClassNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("CallStaticObjectMethodV"),
                    env!(emulator),
                    clz_id,
                    method_id
                );
        ...
        match object {
            ...
            _ => {
                if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                    debug!(
                        "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => RefNotClass",
                        Color::Yellow.paint("JNI:"),
                        Color::Blue.paint("CallStaticObjectMethodV"),
                        env!(emulator),
                        clz_id,
    ...
    let result = dalvik!(emulator)
        ...
        .call_method_v(
            dalvik!(emulator),
            MethodAcc::STATIC | MethodAcc::OBJECT,
            &class,
            method,
            None,
            &mut va_list,
...
fn CallStaticObjectMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallStaticObjectMethodV, name)
}
...
fn CallStaticIntMethodV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => ClassNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("CallStaticIntMethodV"),
                    env!(emulator),
                    clz_id,
                    method_id
                );
    ...
    if method.is_none() {
        ...
        unreachable!(
            "CallStaticIntMethodV: Method not found, class = {}, method = {}",
            clz_id, method_id
        );
    }
    ...
    let result = dalvik!(emulator)
        ...
        .call_method_v(
            dalvik!(emulator),
            MethodAcc::STATIC | MethodAcc::INT,
            &class,
            method,
            None,
            &mut va_list,
...
fn CallStaticIntMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallStaticIntMethodV, name)
}
...
fn CallStaticVoidMethodV<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x}) => ClassNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("CallStaticVoidMethodV"),
                    env!(emulator),
                    clz_id,
                    method_id
                );
    ...
    dalvik!(emulator)
        ...
        .call_method_v(
            dalvik!(emulator),
            MethodAcc::STATIC | MethodAcc::VOID,
            &class,
            method,
            None,
            &mut va_list,
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(env = 0x{:x}, class = 0x{:x}, method = 0x{:x})",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("CallStaticVoidMethodV"),
            env!(emulator),
            clz_id,
    ...
    if clz.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, name = {}, signature = {}) => ClassNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStaticFieldID"),
                env!(emulator),
                clz_id,
                field_name,
                signature
...
fn GetStaticObjectField<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if jni::get_flag_id(clz_id) == JNI_FLAG_REF {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, class = 0x{:x}, field = 0x{:x}) => ClassNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetStaticObjectField"),
                    env!(emulator),
                    clz_id,
                    field_id
                );
        ...
        match object {
            ...
            _ => {
                if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                    debug!(
                        "{} {}(env = 0x{:x}, class = 0x{:x}, field = 0x{:x}) => RefNotClass",
                        Color::Yellow.paint("JNI:"),
                        Color::Blue.paint("GetStaticObjectField"),
                        env!(emulator),
                        clz_id,
    ...
    if field.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, field = 0x{:x}) => FieldNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStaticObjectField"),
                env!(emulator),
                clz_id,
    ...
    if object.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, field = 0x{:x}) => JNI_NULL",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStaticObjectField"),
                env!(emulator),
                clz_id,
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}, field = 0x{:x}) => 0x{:X}",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStaticObjectField"),
                env!(emulator),
                clz_id,
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, class = 0x{:x}, field = 0x{:x}) => ClassNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetStaticIntField"),
                    env!(emulator),
                    clz_id,
                    field_id
                );
    ...
    if value.is_none() {
        ...
        let value = match value {
            ...
            _ => {
                if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                    debug!(
                        "{} {}(env=0x{:x}, class=0x{:x}, field=0x{:x}) => NotInt",
                        Color::Yellow.paint("JNI:"),
                        Color::Blue.paint("GetStaticIntField"),
                        env!(emulator),
                        clz_id,
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env=0x{:x}, class=0x{:x}, field=0x{:x}) => {}",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStaticIntField"),
                env!(emulator),
                clz_id,
...
fn GetStringLength<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    let object = if flag == JNI_FLAG_OBJECT {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}) => NotObject",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStringLength"),
                env!(emulator),
                string
            );
        }
    ...
    if object.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}) => StringNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStringLength"),
                env!(emulator),
                string
    ...
    let len = match object {
        ...
        _ => {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, string = 0x{:x}) => NotString",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetStringLength"),
                    env!(emulator),
                    string
...
fn NewStringUTF<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if cp == 0 {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, utf = NULL) => NULL",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("NewStringUTF"),
                env!(emulator)
            );
        }
        return RET(JNI_NULL);
    ...
    let object = if flag == JNI_FLAG_OBJECT {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}) => NotObject",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStringUTFLength"),
                env!(emulator),
                string
            );
        }
...
fn GetStringUTFChars<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    let object = if flag == JNI_FLAG_OBJECT {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}, isCopy = 0x{:x}) => NotObject",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStringUTFChars"),
                env!(emulator),
                string,
                is_copy
            );
    ...
    if object.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}, isCopy = 0x{:x}) => StringNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("GetStringUTFChars"),
                env!(emulator),
                string,
    ...
    let utf = match object {
        ...
        _ => {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, string = 0x{:x}, isCopy = 0x{:x}) => NotString",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetStringUTFChars"),
                    env!(emulator),
                    string,
    ...
    emulator
        .backend
        .mem_write(dest.addr, buf.as_slice())
        .unwrap();
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(env = 0x{:x}, string = 0x{:x}, isCopy = 0x{:x}) => 0x{:X}",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("GetStringUTFChars"),
            env!(emulator),
            string,
...
fn ReleaseStringUTFChars<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    let object = if flag == JNI_FLAG_OBJECT {
        ...
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}, utf = 0x{:x}) => NotObject",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("ReleaseStringUTFChars"),
                env!(emulator),
                string,
                utf
            );
    ...
    if object.is_none() {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, string = 0x{:x}, utf = 0x{:x}) => StringNotFound",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("ReleaseStringUTFChars"),
                env!(emulator),
                string,
    ...
    let str_len = match object {
        DvmObject::String(str) => {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, string = 0x{:x}, utf = 0x{:x}) => Ok",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("ReleaseStringUTFChars"),
                    env!(emulator),
                    string,
        ...
        _ => {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, string = 0x{:x}, utf = 0x{:x}) => NotString",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("ReleaseStringUTFChars"),
                    env!(emulator),
                    string,
...
fn GetArrayLength<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    let object = if flag == JNI_FLAG_OBJECT {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, array = 0x{:x}) => ArrayNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetArrayLength"),
                    env!(emulator),
                    array
                );
            }
...
fn GetObjectArrayElement<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    let object = if flag == JNI_FLAG_OBJECT {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, array = 0x{:x}, index = {}) => ArrayNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetObjectArrayElement"),
                    env!(emulator),
                    array_id,
                    index
                );
    ...
    match object {
        DvmObject::ObjectArray(_, array) => {
            if index < 0 || index as usize >= array.len() {
                if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                    debug!(
                        "{} {}(env = 0x{:x}, array = 0x{:x}, index = {}) => ArrayIndexOutOfBounds",
                        Color::Yellow.paint("JNI:"),
                        Color::Blue.paint("GetObjectArrayElement"),
                        env!(emulator),
                        array_id,
            ...
            if let Some(object) = array.get(index as usize) {
                if let Some(object) = object {
                    ...
                    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                        debug!(
                            "{} {}(env = 0x{:x}, array = 0x{:x}, index = {}) => 0x{:X}",
                            Color::Yellow.paint("JNI:"),
                            Color::Blue.paint("GetObjectArrayElement"),
                            env!(emulator),
                            array_id,
                    ...
                    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                        debug!(
                            "{} {}(env = 0x{:x}, array = 0x{:x}, index = {}) => NULL",
                            Color::Yellow.paint("JNI:"),
                            Color::Blue.paint("GetObjectArrayElement"),
                            env!(emulator),
                            array_id,
        ...
        _ => {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, array = 0x{:x}, index = {}) => NotArray",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetObjectArrayElement"),
                    env!(emulator),
                    array_id,
...
fn GetByteArrayElements<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
        ...
        if object.is_none() {
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, array = 0x{:x}) => ArrayNotFound",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetByteArrayElements"),
                    env!(emulator),
                    array
                );
            }
    ...
    match object {
        DvmObject::ByteArray(data) => {
            ...
            if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
                debug!(
                    "{} {}(env = 0x{:x}, array = 0x{:x}, is_copy = 0x{:X}) => 0x{:X}",
                    Color::Yellow.paint("JNI:"),
                    Color::Blue.paint("GetByteArrayElements"),
                    env!(emulator),
                    array,
...
fn ReleaseByteArrayElements<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(env = 0x{:x}, array = 0x{:x}, elems = 0x{:x}, mode = {}) => OK",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("ReleaseByteArrayElements"),
            env!(emulator),
            array,
            elems,
            mode
    ...
    match mode {
        0 => {
            let new_data = emulator
                .backend
                .mem_read_as_vec(elems, array_object.len())
                .unwrap();
...
fn CallStaticVoidMethod<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    with_synthesized_varargs(emulator, CallStaticVoidMethodV, name)
}
...
fn GetByteArrayRegion<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if flag != JNI_FLAG_REF && flag != JNI_FLAG_OBJECT {
        ...
        return FUCK(anyhow!(
            "_GetByteArrayRegion: input array pointer is invalid"
        ));
    }
    ...
    let data = match flag {
        JNI_FLAG_REF => {
            ...
            if ref_data.is_none() {
                ...
                return FUCK(anyhow!(
                    "_GetByteArrayRegion: input array pointer is not found in global ref pool"
                ));
            }
        ...
        JNI_FLAG_OBJECT => {
            ...
            if ref_data.is_none() {
                ...
                return FUCK(anyhow!(
                    "_GetByteArrayRegion: input array pointer is not found in local ref pool"
                ));
            }
            match ref_data.unwrap() {
                DvmObject::ByteArray(bytes) => bytes,
    ...
    if option_env!("PRINT_JNI_CALLS_EX").unwrap_or("") == "1" {
        debug!(
            ...
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("GetByteArrayRegion"),
            env!(emulator),
            array,
            start,
            length,
...
fn SetByteArrayRegion<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if flag != JNI_FLAG_REF && flag != JNI_FLAG_OBJECT {
        ...
        return FUCK(anyhow!(
            "_SetByteArrayRegion: input array pointer is invalid"
        ));
    }
    ...
    if flag == JNI_FLAG_REF {
        ...
        if ref_data.is_none() {
            ...
            return FUCK(anyhow!(
                "_SetByteArrayRegion: input array pointer is not found in global ref pool"
            ));
        }
        match ref_data.unwrap() {
        ...
        if ref_data.is_none() {
            ...
            return FUCK(anyhow!(
                "_SetByteArrayRegion: input array pointer is not found in local ref pool"
            ));
        }
        match ref_data.unwrap() {
            DvmObject::ByteArray(bytes) => {
...
fn MicroGetJavaVM<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    emulator
        .backend
        .mem_write(vm_ptr, &vm.to_le_bytes())
        .unwrap();
    RET(JNI_OK)
}
...
fn RegisterNatives<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    ...
    if n_methods == 0 {
        if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
            debug!(
                "{} {}(env = 0x{:x}, class = 0x{:x}) => NoAnyMethod",
                Color::Yellow.paint("JNI:"),
                Color::Blue.paint("RegisterNatives"),
                env!(emulator),
                clz_id
            );
        }
    ...
    for i in 0..n_methods {
        emulator
            .backend
            .mem_read(methods + i * JNINATIVE_METHOD_SIZE as u64, &mut buf)
            .unwrap();
        ...
        start_id += 1;
        members_list.push(DvmMember::Method(DvmMethod::new(
            clz_id + start_id,
            clz_id,
            name,
            signature,
...
fn ExceptionCheck<T: Clone>(name: &str, emulator: &AndroidEmulator<T>) -> SvcCallResult {
    let dvm = emulator.inner_mut().dalvik.as_ref().unwrap();
    let ret = if dvm.throwable.is_some() {
        JNI_TRUE
    } else {
        JNI_FALSE
    };
    if option_env!("PRINT_JNI_CALLS").unwrap_or("") == "1" {
        debug!(
            "{} {}(env = 0x{:x}) => {}",
            Color::Yellow.paint("JNI:"),
            Color::Blue.paint("ExceptionCheck"),
            env!(emulator),
            ret
        );
    }
...
pub fn initialize_env<T: Clone>(svc_memory: &mut SvcMemory<T>) -> u64 {
    ...
    let _from_reflected_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_FromReflectedMethod",
        NoImplementedHandler,
    ));
    let _from_reflected_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_FromReflectedField",
        NoImplementedHandler,
    ));
    let _to_reflected_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ToReflectedMethod",
        NoImplementedHandler,
    ));
    let _get_superclass =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetSuperclass", NoImplementedHandler));
    let _is_assignable_from = svc_memory.register_svc(SimpleArm64Svc::new(
        "_IsAssignableFrom",
        NoImplementedHandler,
    ));
    let _to_reflected_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ToReflectedField",
        NoImplementedHandler,
    ));
    let _throw = svc_memory.register_svc(SimpleArm64Svc::new("_Throw", NoImplementedHandler));
    let _throw_new =
        svc_memory.register_svc(SimpleArm64Svc::new("_ThrowNew", NoImplementedHandler));
    let _exception_occurred = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ExceptionOccurred",
        NoImplementedHandler,
    ));
    ...
    let _fatal_error =
        svc_memory.register_svc(SimpleArm64Svc::new("_FatalError", NoImplementedHandler));
    let _push_local_frame =
        svc_memory.register_svc(SimpleArm64Svc::new("_PushLocalFrame", NoImplementedHandler));
    let _pop_local_frame =
        svc_memory.register_svc(SimpleArm64Svc::new("_PopLocalFrame", NoImplementedHandler));
    let _new_global_ref =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewGlobalRef", NewGlobalRef));
    let _delete_global_ref =
        svc_memory.register_svc(SimpleArm64Svc::new("_DeleteGlobalRef", DeleteGlobalRef));
    let _delete_local_ref =
        svc_memory.register_svc(SimpleArm64Svc::new("_DeleteLocalRef", DeleteLocalRef));
    let _is_same_object =
        svc_memory.register_svc(SimpleArm64Svc::new("_IsSameObject", NoImplementedHandler));
    let _new_local_ref =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewLocalRef", NoImplementedHandler));
    let _ensure_local_capacity = svc_memory.register_svc(SimpleArm64Svc::new(
        "_EnsureLocalCapacity",
        NoImplementedHandler,
    ));
    let _alloc_object =
    ...
    let _get_object_class =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetObjectClass", GetObjectClass));
    let _is_instance_of =
        svc_memory.register_svc(SimpleArm64Svc::new("_IsInstanceOf", NoImplementedHandler));
    let _get_method_id = svc_memory.register_svc(SimpleArm64Svc::new("_GetMethodID", GetMethodID));
    let _call_object_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallObjectMethod", CallObjectMethod));
    let _call_object_method_v =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallObjectMethodV", CallObjectMethodV));
    let _call_object_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallObjectMethodA",
        NoImplementedHandler,
    ));
    let _call_boolean_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallBooleanMethod", CallBooleanMethod));
    let _call_boolean_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallBooleanMethodV",
        CallBooleanMethodV,
    ));
    let _call_boolean_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallBooleanMethodA",
        NoImplementedHandler,
    ));
    let _call_byte_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallByteMethod", NoImplementedHandler));
    let _call_byte_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallByteMethodV",
        NoImplementedHandler,
    ));
    let _call_byte_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallByteMethodA",
        NoImplementedHandler,
    ));
    let _call_char_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallCharMethod", NoImplementedHandler));
    let _call_char_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallCharMethodV",
        NoImplementedHandler,
    ));
    let _call_char_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallCharMethodA",
        NoImplementedHandler,
    ));
    let _call_short_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallShortMethod",
        NoImplementedHandler,
    ));
    let _call_short_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallShortMethodV",
        NoImplementedHandler,
    ));
    let _call_short_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallShortMethodA",
        NoImplementedHandler,
    ));
    let _call_int_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallIntMethod", CallIntMethod));
    let _call_int_method_v =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallIntMethodV", CallIntMethodV));
    let _call_int_method_a =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallIntMethodA", NoImplementedHandler));
    let _call_long_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallLongMethod", NoImplementedHandler));
    let _call_long_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallLongMethodV",
        NoImplementedHandler,
    ));
    let _call_long_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallLongMethodA",
        NoImplementedHandler,
    ));
    let _call_float_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallFloatMethod",
        NoImplementedHandler,
    ));
    let _call_float_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallFloatMethodV",
        NoImplementedHandler,
    ));
    let _call_float_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallFloatMethodA",
        NoImplementedHandler,
    ));
    let _call_double_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallDoubleMethod",
        NoImplementedHandler,
    ));
    let _call_double_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallDoubleMethodV",
        NoImplementedHandler,
    ));
    let _call_double_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallDoubleMethodA",
        NoImplementedHandler,
    ));
    let _call_void_method =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallVoidMethod", CallVoidMethod));
    let _call_void_method_v =
        svc_memory.register_svc(SimpleArm64Svc::new("_CallVoidMethodV", CallVoidMethodV));
    let _call_void_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallVoidMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_object_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualObjectMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_object_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualObjectMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_object_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualObjectMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_boolean_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualBooleanMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_boolean_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualBooleanMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_boolean_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualBooleanMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_byte_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualByteMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_byte_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualByteMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_byte_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualByteMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_char_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualCharMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_char_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualCharMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_char_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualCharMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_short_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualShortMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_short_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualShortMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_short_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualShortMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_int_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualIntMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_int_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualIntMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_int_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualIntMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_long_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualLongMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_long_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualLongMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_long_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualLongMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_float_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualFloatMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_float_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualFloatMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_float_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualFloatMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_double_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualDoubleMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_double_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualDoubleMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_double_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualDoubleMethodA",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_void_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualVoidMethod",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_void_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualVoidMethodV",
        NoImplementedHandler,
    ));
    let _call_nonvirtual_void_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallNonvirtualVoidMethodA",
        NoImplementedHandler,
    ));
    let _get_field_id = svc_memory.register_svc(SimpleArm64Svc::new("_GetFieldID", GetFieldID));
    let _get_object_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetObjectField", GetObjectField));
    let _get_boolean_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetBooleanField",
        NoImplementedHandler,
    ));
    let _get_byte_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetByteField", NoImplementedHandler));
    let _get_char_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetCharField", NoImplementedHandler));
    let _get_short_field =
    ...
    let _get_double_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetDoubleField", NoImplementedHandler));
    let _set_object_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetObjectField", SetObjectField));
    let _set_boolean_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetBooleanField",
        NoImplementedHandler,
    ));
    let _set_byte_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetByteField", NoImplementedHandler));
    let _set_char_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetCharField", NoImplementedHandler));
    let _set_short_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetShortField", NoImplementedHandler));
    let _set_int_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetIntField", NoImplementedHandler));
    let _set_long_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetLongField", NoImplementedHandler));
    let _set_float_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetFloatField", NoImplementedHandler));
    let _set_double_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_SetDoubleField", NoImplementedHandler));
    let _get_static_method_id =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetStaticMethodID", GetStaticMethodID));
    let _call_static_object_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticObjectMethod",
        CallStaticObjectMethod,
    ));
    let _call_static_object_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticObjectMethodV",
        CallStaticObjectMethodV,
    ));
    let _call_static_object_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticObjectMethodA",
        NoImplementedHandler,
    ));
    let _call_static_boolean_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticBooleanMethod",
        NoImplementedHandler,
    ));
    let _call_static_boolean_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticBooleanMethodV",
        NoImplementedHandler,
    ));
    let _call_static_boolean_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticBooleanMethodA",
        NoImplementedHandler,
    ));
    let _call_static_byte_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticByteMethod",
        NoImplementedHandler,
    ));
    let _call_static_byte_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticByteMethodV",
        NoImplementedHandler,
    ));
    let _call_static_byte_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticByteMethodA",
        NoImplementedHandler,
    ));
    let _call_static_char_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticCharMethod",
        NoImplementedHandler,
    ));
    let _call_static_char_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticCharMethodV",
        NoImplementedHandler,
    ));
    let _call_static_char_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticCharMethodA",
        NoImplementedHandler,
    ));
    let _call_static_short_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticShortMethod",
        NoImplementedHandler,
    ));
    let _call_static_short_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticShortMethodV",
        NoImplementedHandler,
    ));
    let _call_static_short_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticShortMethodA",
        NoImplementedHandler,
    ));
    let _call_static_int_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticIntMethod",
        CallStaticIntMethod,
    ));
    let _call_static_int_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticIntMethodV",
        CallStaticIntMethodV,
    ));
    let _call_static_int_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticIntMethodA",
        NoImplementedHandler,
    ));
    let _call_static_long_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticLongMethod",
        NoImplementedHandler,
    ));
    let _call_static_long_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticLongMethodV",
        NoImplementedHandler,
    ));
    let _call_static_long_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticLongMethodA",
        NoImplementedHandler,
    ));
    let _call_static_float_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticFloatMethod",
        NoImplementedHandler,
    ));
    let _call_static_float_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticFloatMethodV",
        NoImplementedHandler,
    ));
    let _call_static_float_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticFloatMethodA",
        NoImplementedHandler,
    ));
    let _call_static_double_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticDoubleMethod",
        NoImplementedHandler,
    ));
    let _call_static_double_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticDoubleMethodV",
        NoImplementedHandler,
    ));
    let _call_static_double_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticDoubleMethodA",
        NoImplementedHandler,
    ));
    let _call_static_void_method = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticVoidMethod",
        CallStaticVoidMethod,
    ));
    let _call_static_void_method_v = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticVoidMethodV",
        CallStaticVoidMethodV,
    ));
    let _call_static_void_method_a = svc_memory.register_svc(SimpleArm64Svc::new(
        "_CallStaticVoidMethodA",
        NoImplementedHandler,
    ));
    let _get_static_field_id =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetStaticFieldID", GetStaticFieldID));
    let _get_static_object_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticObjectField",
        GetStaticObjectField,
    ));
    let _get_static_boolean_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticBooleanField",
        NoImplementedHandler,
    ));
    let _get_static_byte_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticByteField",
        NoImplementedHandler,
    ));
    let _get_static_char_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticCharField",
        NoImplementedHandler,
    ));
    let _get_static_short_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticShortField",
        NoImplementedHandler,
    ));
    let _get_static_int_field =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetStaticIntField", GetStaticIntField));
    let _get_static_long_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticLongField",
        NoImplementedHandler,
    ));
    let _get_static_float_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticFloatField",
        NoImplementedHandler,
    ));
    let _get_static_double_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStaticDoubleField",
        NoImplementedHandler,
    ));
    let _set_static_object_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticObjectField",
        NoImplementedHandler,
    ));
    let _set_static_boolean_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticBooleanField",
        NoImplementedHandler,
    ));
    let _set_static_byte_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticByteField",
        NoImplementedHandler,
    ));
    let _set_static_char_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticCharField",
        NoImplementedHandler,
    ));
    let _set_static_short_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticShortField",
        NoImplementedHandler,
    ));
    let _set_static_int_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticIntField",
        NoImplementedHandler,
    ));
    let _set_static_long_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticLongField",
        NoImplementedHandler,
    ));
    let _set_static_float_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticFloatField",
        NoImplementedHandler,
    ));
    let _set_static_double_field = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetStaticDoubleField",
        NoImplementedHandler,
    ));
    let _new_string =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewString", NoImplementedHandler));
    let _get_string_length =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetStringLength", GetStringLength));
    let _get_string_chars =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetStringChars", NoImplementedHandler));
    let _release_string_chars = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseStringChars",
        NoImplementedHandler,
    ));
    let _new_string_utf =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewStringUTF", NewStringUTF));
    let _get_string_utf_length = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStringUTFLength",
        GetStringUTFLength,
    ));
    let _get_string_utf_chars =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetStringUTFChars", GetStringUTFChars));
    let _release_string_utf_chars = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseStringUTFChars",
        ReleaseStringUTFChars,
    ));
    let _get_array_length =
        svc_memory.register_svc(SimpleArm64Svc::new("_GetArrayLength", GetArrayLength));
    let _new_object_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewObjectArray", NoImplementedHandler));
    let _get_object_array_element = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetObjectArrayElement",
        GetObjectArrayElement,
    ));
    let _set_object_array_element = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetObjectArrayElement",
        NoImplementedHandler,
    ));
    let _new_boolean_array = svc_memory.register_svc(SimpleArm64Svc::new(
        "_NewBooleanArray",
        NoImplementedHandler,
    ));
    let _new_byte_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewByteArray", NewByteArray));
    let _new_char_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewCharArray", NoImplementedHandler));
    let _new_short_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewShortArray", NoImplementedHandler));
    let _new_int_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewIntArray", NoImplementedHandler));
    let _new_long_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewLongArray", NoImplementedHandler));
    let _new_float_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewFloatArray", NoImplementedHandler));
    let _new_double_array =
        svc_memory.register_svc(SimpleArm64Svc::new("_NewDoubleArray", NoImplementedHandler));
    let _get_boolean_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetBooleanArrayElements",
        NoImplementedHandler,
    ));
    let _get_byte_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetByteArrayElements",
        GetByteArrayElements,
    ));
    let _get_char_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetCharArrayElements",
        NoImplementedHandler,
    ));
    let _get_short_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetShortArrayElements",
        NoImplementedHandler,
    ));
    let _get_int_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetIntArrayElements",
        NoImplementedHandler,
    ));
    let _get_long_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetLongArrayElements",
        NoImplementedHandler,
    ));
    let _get_float_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetFloatArrayElements",
        NoImplementedHandler,
    ));
    let _get_double_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetDoubleArrayElements",
        NoImplementedHandler,
    ));
    let _release_boolean_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseBooleanArrayElements",
        NoImplementedHandler,
    ));
    let _release_byte_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseByteArrayElements",
        ReleaseByteArrayElements,
    ));
    let _release_char_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseCharArrayElements",
        NoImplementedHandler,
    ));
    let _release_short_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseShortArrayElements",
        NoImplementedHandler,
    ));
    let _release_int_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseIntArrayElements",
        NoImplementedHandler,
    ));
    let _release_long_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseLongArrayElements",
        NoImplementedHandler,
    ));
    let _release_float_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseFloatArrayElements",
        NoImplementedHandler,
    ));
    let _release_double_array_elements = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseDoubleArrayElements",
        NoImplementedHandler,
    ));
    let _get_boolean_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetBooleanArrayRegion",
        NoImplementedHandler,
    ));
    let _get_byte_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetByteArrayRegion",
        GetByteArrayRegion,
    ));
    let _get_char_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetCharArrayRegion",
        NoImplementedHandler,
    ));
    let _get_short_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetShortArrayRegion",
        NoImplementedHandler,
    ));
    let _get_int_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetIntArrayRegion",
        NoImplementedHandler,
    ));
    let _get_long_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetLongArrayRegion",
        NoImplementedHandler,
    ));
    let _get_float_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetFloatArrayRegion",
        NoImplementedHandler,
    ));
    let _get_double_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetDoubleArrayRegion",
        NoImplementedHandler,
    ));
    let _set_boolean_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetBooleanArrayRegion",
        NoImplementedHandler,
    ));
    let _set_byte_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetByteArrayRegion",
        SetByteArrayRegion,
    ));
    let _set_char_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetCharArrayRegion",
        NoImplementedHandler,
    ));
    let _set_short_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetShortArrayRegion",
        NoImplementedHandler,
    ));
    let _set_int_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetIntArrayRegion",
        NoImplementedHandler,
    ));
    let _set_long_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetLongArrayRegion",
        NoImplementedHandler,
    ));
    let _set_float_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetFloatArrayRegion",
        NoImplementedHandler,
    ));
    let _set_double_array_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_SetDoubleArrayRegion",
        NoImplementedHandler,
    ));
    let _register_natives =
        svc_memory.register_svc(SimpleArm64Svc::new("_RegisterNatives", RegisterNatives));
    let _unregister_natives = svc_memory.register_svc(SimpleArm64Svc::new(
        "_UnregisterNatives",
        NoImplementedHandler,
    ));
    let _monitor_enter =
        svc_memory.register_svc(SimpleArm64Svc::new("_MonitorEnter", NoImplementedHandler));
    let _monitor_exit =
        svc_memory.register_svc(SimpleArm64Svc::new("_MonitorExit", NoImplementedHandler));
    let _get_java_vm = svc_memory.register_svc(SimpleArm64Svc::new("_GetJavaVM", MicroGetJavaVM));
    let _get_string_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStringRegion",
        NoImplementedHandler,
    ));
    let _get_string_utf_region = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStringUTFRegion",
        NoImplementedHandler,
    ));
    let _get_primitive_array_critical = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetPrimitiveArrayCritical",
        NoImplementedHandler,
    ));
    let _release_primitive_array_critical = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleasePrimitiveArrayCritical",
        NoImplementedHandler,
    ));
    let _get_string_critical = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetStringCritical",
        NoImplementedHandler,
    ));
    let _release_string_critical = svc_memory.register_svc(SimpleArm64Svc::new(
        "_ReleaseStringCritical",
        NoImplementedHandler,
    ));
    let _new_weak_global_ref = svc_memory.register_svc(SimpleArm64Svc::new(
        "_NewWeakGlobalRef",
        NoImplementedHandler,
    ));
    let _delete_weak_global_ref = svc_memory.register_svc(SimpleArm64Svc::new(
        "_DeleteWeakGlobalRef",
        NoImplementedHandler,
    ));
    let _exception_check =
        svc_memory.register_svc(SimpleArm64Svc::new("_ExceptionCheck", ExceptionCheck));
    let _new_direct_byte_buffer = svc_memory.register_svc(SimpleArm64Svc::new(
        "_NewDirectByteBuffer",
        NoImplementedHandler,
    ));
    let _get_direct_buffer_address = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetDirectBufferAddress",
        NoImplementedHandler,
    ));
    let _get_direct_buffer_capacity = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetDirectBufferCapacity",
        NoImplementedHandler,
    ));
    let _get_object_ref_type = svc_memory.register_svc(SimpleArm64Svc::new(
        "_GetObjectRefType",
        NoImplementedHandler,
    ));
    ...
    java_env
        .write_u64_with_offset(0, imp.addr)
        .expect("write_u64_with_offset failed: _JNIEnv.impl");
