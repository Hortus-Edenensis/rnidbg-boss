...
impl ModuleSymbol {
            ...
            if let Ok(elf_symbol) = elf_symbol {
                if !elf_symbol.is_undefined() {
                    ...
                    if binding == BINDING_GLOBAL || binding == BINDING_WEAK {
                        let hash =
                            tool::calculate_hash(&format!("{}#{}", module.name, symbol_name));
                        if cache_hook.contains_key(&hash) {
                            return Ok(ModuleSymbol::new(
                                self.so_name.to_string(),
                                WEAK_BASE,
    ...
    pub(crate) fn relocation<'a, T: Clone>(
        &self,
        owner: &mut LinuxModule<'a, T>,
        elf_file: &ElfFile,
        backend: &Backend<'a, T>,
    ) -> anyhow::Result<()> {
        ...
        let value = if self.load_base == WEAK_BASE {
            ...
            self.load_base as u64
                + self.symbol.as_ref().map(|s| s.value as u64).unwrap_or(0u64)
                + self.offset
        };
    ...
    pub(crate) fn relocation_ex2<'a, T: Clone>(
        &self,
        backend: &Backend<'a, T>,
        symbol: &ElfSymbol,
    ) -> anyhow::Result<()> {
    ...
    pub(crate) fn relocation_ex<'a, T: Clone>(
        &self,
        resolved_symbol: &mut IndexMap<String, ModuleSymbol, RandomState>,
        elf_file: &ElfFile,
        backend: &Backend<'a, T>,
    ) -> anyhow::Result<()> {
        if let Some(symbol) = self.symbol.as_ref() {
