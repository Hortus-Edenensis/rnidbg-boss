...
function getJavaJsonBridge() {
    ...
    try {
        ...
        bridge.MapCls = tryLoadJavaClass("java.util.Map");
        bridge.HashMapCls = tryLoadJavaClass("java.util.HashMap");
        bridge.GsonCls = null;
...
function parseJavaMapPassive(mapObj, depth) {
    ...
    var mapView = mapObj;
    if (bridge) {
        mapView = tryCastTo(mapObj, bridge.MapCls) || tryCastTo(mapObj, bridge.HashMapCls) || mapObj;
    }
    var d = depth || 0;
    if (d > 8) {
        return {};
    ...
    var out = {};
    try {
        var it = mapView.entrySet().iterator();
        while (it && it.hasNext()) {
            var entry = it.next();
    ...
    try {
        var keyIt = mapView.keySet().iterator();
        while (keyIt && keyIt.hasNext()) {
            var keyObj = keyIt.next();
            var keyText = safeToString(keyObj);
            out[keyText] = parseOrgJsonPassive(mapView.get(keyObj), d + 1);
        }
        if (hasMeaningfulJson(out)) {
            return out;
        }
    ...
    try {
        var mapText = safeToString(mapView.toString());
...
function tryJsonifyJavaMap(mapObj) {
    ...
    var mapView = tryCastTo(mapObj, bridge.MapCls) || tryCastTo(mapObj, bridge.HashMapCls) || mapObj;
    try {
        var jsonObj = bridge.JSONObjectCls.$new(mapView);
        if (!jsonObj) {
            return null;
        }
