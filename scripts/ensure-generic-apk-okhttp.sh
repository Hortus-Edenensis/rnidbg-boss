...
if [[ -z "$OUT_DIR" ]]; then
  OUT_DIR="$REPO_ROOT/target/generic-apk-okhttp"
fi

JAVA_SRC="$REPO_ROOT/java/generic_apk_okhttp/src/rnidbg/generic/okhttp/GenericApkOkHttpCli.java"
...
DEX2JAR_BIN=""
JAR_PATH="$OUT_DIR/app-okhttp.jar"
...
if [[ ! -f "$JAVA_SRC" ]]; then
  echo "java source not found: $JAVA_SRC" >&2
  exit 1
fi

NEEDS_COMPILE=0
if [[ ! -f "$STAMP_PATH" || "$JAR_PATH" -nt "$STAMP_PATH" || "$JAVA_SRC" -nt "$STAMP_PATH" ]]; then
  NEEDS_COMPILE=1
fi
