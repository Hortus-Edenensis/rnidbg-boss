...
final class MatrixUtils {
    ...
    public static ImmutableList<float[]> clipConvexPolygonToNdcRange(ImmutableList<float[]> immutableList) {
        ...
        ImmutableList.a aVarL = new ImmutableList.a().l(immutableList);
        float[][] fArr = NDC_CUBE;
        int length = fArr.length;
        int i = 0;
        while (i < length) {
            float[] fArr2 = fArr[i];
            ImmutableList immutableListE = aVarL.e();
            ImmutableList.a aVar = new ImmutableList.a();
            for (int i2 = 0; i2 < immutableListE.size(); i2++) {
                float[] fArr3 = (float[]) immutableListE.get(i2);
                float[] fArr4 = (float[]) immutableListE.get(((immutableListE.size() + i2) - 1) % immutableListE.size());
                if (isInsideClippingHalfSpace(fArr3, fArr2)) {
                    if (!isInsideClippingHalfSpace(fArr4, fArr2)) {
                        ...
                        if (!Arrays.equals(fArr3, fArrComputeIntersectionPoint)) {
                            aVar.a(fArrComputeIntersectionPoint);
                        }
                    }
                    aVar.a(fArr3);
                    ...
                    if (!Arrays.equals(fArr4, fArrComputeIntersectionPoint2)) {
                        aVar.a(fArrComputeIntersectionPoint2);
                    }
                }
            }
            i++;
