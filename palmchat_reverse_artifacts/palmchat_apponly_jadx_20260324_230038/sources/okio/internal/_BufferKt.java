...
public final class _BufferKt {
    ...
    public static final int commonNext(Buffer.UnsafeCursor unsafeCursor) {
        ...
        return unsafeCursor.seek(j2 == -1 ? 0L : j2 + ((long) (unsafeCursor.end - unsafeCursor.start)));
    }
    ...
    */
    ...
    public static final <T> T seek(Buffer buffer, long j, Function2<? super Segment, ? super Long, ? extends T> lambda) {
        ...
        Segment segment = buffer.head;
        if (segment == null) {
            return lambda.mo5invoke(null, -1L);
        }
        if (buffer.size() - j < j) {
            long size = buffer.size();
            while (size > j) {
            ...
            return lambda.mo5invoke(segment, Long.valueOf(size));
        }
        long j2 = 0;
        while (true) {
            long j3 = ((long) (segment.limit - segment.pos)) + j2;
            if (j3 > j) {
                return lambda.mo5invoke(segment, Long.valueOf(j2));
            }
            segment = segment.next;
            Intrinsics.checkNotNull(segment);
            j2 = j3;
    ...
    public static final long commonIndexOf(Buffer buffer, ByteString bytes, long j) {
        ...
        if (j3 >= 0) {
            ...
            while (size < size5) {
                ...
                for (int i2 = (int) ((((long) segment.pos) + j6) - size); i2 < iMin2; i2++) {
