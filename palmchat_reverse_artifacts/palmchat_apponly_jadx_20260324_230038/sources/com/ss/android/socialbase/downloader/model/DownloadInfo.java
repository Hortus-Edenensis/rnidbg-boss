...
public class DownloadInfo implements Parcelable {
    ...
    private com.ss.android.socialbase.downloader.constants.u asyncHandleStatus;
    ...
    private com.ss.android.socialbase.downloader.constants.nr byteInvalidRetryStatus;
    ...
    private volatile List<pb> tempFileSaveCompleteCallbacks;
    ...
    public static class u {
        ...
        private EnqueueType pb = EnqueueType.ENQUEUE_NONE;
        ...
        public u bg(boolean z) {
            this.mh = z;
            return this;
        }

        public u bq(boolean z) {
            this.yd = z;
            return this;
        }
        ...
        public u mv(boolean z) {
            this.xg = z;
            return this;
        }

        public u my(boolean z) {
            this.oa = z;
            return this;
        }
        ...
        public u u(EnqueueType enqueueType) {
            this.pb = enqueueType;
            return this;
        }
    ...
    private void convertRetryDelayStatus(int i) {
        x xVar = x.DELAY_RETRY_WAITING;
        if (i == xVar.ordinal()) {
            this.retryDelayStatus = xVar;
            return;
        }
        x xVar2 = x.DELAY_RETRY_DOWNLOADING;
        if (i == xVar2.ordinal()) {
            this.retryDelayStatus = xVar2;
            return;
        }
        x xVar3 = x.DELAY_RETRY_DOWNLOADED;
        if (i == xVar3.ordinal()) {
            this.retryDelayStatus = xVar3;
        } else {
            this.retryDelayStatus = x.DELAY_RETRY_NONE;
        }
    }
    ...
    private void ensureSpData() {
        if (this.spData == null) {
            ...
            if (contextOa != null) {
                String string = com.bytedance.sdk.openadsdk.api.plugin.nr.nr(contextOa, "sp_download_info", 0).getString(Long.toString(getId()), "");
    ...
    public boolean canReStartAsyncTask() {
        return getStatus() != -3 && this.asyncHandleStatus == com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_WAITING;
    }
    ...
    public boolean canSkipStatusHandler() {
        com.ss.android.socialbase.downloader.constants.u uVar;
        int status = getStatus();
        return status == 7 || this.retryDelayStatus == x.DELAY_RETRY_WAITING || status == 8 || (uVar = this.asyncHandleStatus) == com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_WAITING || uVar == com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_RESTART || this.byteInvalidRetryStatus == com.ss.android.socialbase.downloader.constants.nr.BYTE_INVALID_RETRY_STATUS_RESTART;
    }

    public boolean canStartRetryDelayTask() {
        return isNeedRetryDelay() && getStatus() != -3 && this.retryDelayStatus == x.DELAY_RETRY_WAITING;
    }

    public void changeSkipStatus() {
        com.ss.android.socialbase.downloader.constants.u uVar;
        int status = getStatus();
        if (status == 7 || this.retryDelayStatus == x.DELAY_RETRY_WAITING) {
            setRetryDelayStatus(x.DELAY_RETRY_DOWNLOADING);
        }
        if (status == 8 || (uVar = this.asyncHandleStatus) == com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_WAITING || uVar == com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_RESTART) {
            setAsyncHandleStatus(com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_DOWNLOADING);
        }
        if (this.byteInvalidRetryStatus == com.ss.android.socialbase.downloader.constants.nr.BYTE_INVALID_RETRY_STATUS_RESTART) {
            setByteInvalidRetryStatus(com.ss.android.socialbase.downloader.constants.nr.BYTE_INVALID_RETRY_STATUS_DOWNLOADING);
        }
    }

    public int checkMd5Status() {
        return iz.nr(getSavePath(), getName(), this.md5);
    }

    public boolean checkMd5Valid() {
        return iz.fx(getSavePath(), getName(), this.md5);
    }
    ...
    public void clearSpData() {
        ...
        if (contextOa != null) {
            try {
                com.bytedance.sdk.openadsdk.api.plugin.nr.nr(contextOa, "sp_download_info", 0).edit().remove(Integer.toString(getId())).apply();
            } catch (Throwable unused) {
            }
        }
    }
    ...
    public com.ss.android.socialbase.downloader.constants.u getAsyncHandleStatus() {
    ...
    public com.ss.android.socialbase.downloader.constants.nr getByteInvalidRetryStatus() {
    ...
    public String getConnectionUrl() {
        ...
        if (!this.backUpUrlUsed || (list = this.backUpUrls) == null || list.size() <= 0 || (i = this.curBackUpUrlIndex) < 0 || i >= this.backUpUrls.size()) {
            return (!TextUtils.isEmpty(this.url) && this.url.startsWith(BaseConstants.SCHEME_HTTPS) && this.needHttpsToHttpRetry && this.httpsToHttpRetryUsed) ? this.url.replaceFirst(BaseConstants.SCHEME_HTTPS, HttpHost.DEFAULT_SCHEME_NAME) : str;
        }
    ...
    public List<fx> getExtraHeaders() {
    ...
    public int getId() {
        if (this.id == 0) {
            this.id = com.ss.android.socialbase.downloader.downloader.fx.u(this);
        }
        return this.id;
    }
    ...
    public String getTargetFilePath() {
        return iz.u(this.savePath, this.name);
    }
    ...
    public String getTempFilePath() {
        return iz.u(this.savePath, this.tempPath, this.name);
    }

    public String getTempName() {
        return iz.fx(this.name);
    }

    public String getTempPath() {
        return iz.nr(this.savePath, this.tempPath);
    }
    ...
    public synchronized void handleTempSaveCallback(boolean z, BaseException baseException) {
        ...
        com.ss.android.socialbase.downloader.fx.u.nr(TAG, "handleTempSaveCallback isSuccess " + z + " callback size:" + this.tempFileSaveCompleteCallbacks.size());
        for (pb pbVar : this.tempFileSaveCompleteCallbacks) {
            if (pbVar != null) {
                if (z) {
                    pbVar.u();
                } else {
    ...
    public boolean isChunkBreakpointAvailable() {
        jk jkVarKj;
        if (this.chunkCount > 1 && (jkVarKj = com.ss.android.socialbase.downloader.downloader.fx.kj()) != null) {
            List<nr> listFx = jkVarKj.fx(getId());
            if (listFx == null || listFx.size() != this.chunkCount) {
                return false;
            }
            long jK = 0;
            for (nr nrVar : listFx) {
    ...
    public boolean isDownloadWithWifiValid() {
        return !isOnlyWifi() || iz.nr(com.ss.android.socialbase.downloader.downloader.fx.oa());
    }
    ...
    public boolean isExpiredRedownload() {
        if (com.ss.android.socialbase.downloader.n.u.fx().u("force_close_download_cache_check", 0) != 1) {
            return this.expiredRedownload;
        }
        com.ss.android.socialbase.downloader.fx.u.fx("isExpiredRedownload force to false, reason(global setting) id=" + getId() + " name=" + getName());
        return false;
    }
    ...
    public boolean isFileDataValid() {
        ...
        if (zExists && !zIsDirectory) {
            ...
            if (com.ss.android.socialbase.downloader.n.u.fx().u("fix_file_data_valid")) {
                if (curBytes > 0) {
                ...
                com.ss.android.socialbase.downloader.fx.u.b(TAG, "isFileDataValid: cur = " + curBytes + ",totalBytes =" + this.totalBytes + ",fileLength=" + length);
                return false;
            }
    ...
    public boolean isPauseReserveWithWifiValid() {
        if (this.mDownloadFromReserveWifi) {
            return isPauseReserveOnWifi() && iz.nr(com.ss.android.socialbase.downloader.downloader.fx.oa());
        }
        return true;
    }
    ...
    public void readFromParcel(Parcel parcel) {
        ...
        this.extraHeaders = parcel.createTypedArrayList(fx.CREATOR);
    ...
    public synchronized void registerTempFileSaveCallback(pb pbVar) {
        if (pbVar == null) {
            return;
        }
        try {
            com.ss.android.socialbase.downloader.fx.u.nr(TAG, "registerTempFileSaveCallback");
            if (this.tempFileSaveCompleteCallbacks == null) {
                this.tempFileSaveCompleteCallbacks = new ArrayList();
            }
            if (!this.tempFileSaveCompleteCallbacks.contains(pbVar)) {
                this.tempFileSaveCompleteCallbacks.add(pbVar);
            }
        } catch (Throwable th) {
            pbVar.u(new BaseException(1038, iz.nr(th, "registerTempFileSaveCallback")));
        }
    }
    ...
    public void setAsyncHandleStatus(com.ss.android.socialbase.downloader.constants.u uVar) {
        this.asyncHandleStatus = uVar;
    }
    ...
    public void setByteInvalidRetryStatus(com.ss.android.socialbase.downloader.constants.nr nrVar) {
        this.byteInvalidRetryStatus = nrVar;
    }
    ...
    public void setRetryDelayStatus(x xVar) {
        this.retryDelayStatus = xVar;
    }
    ...
    public ContentValues toContentValues() {
        ...
        contentValues.put(LxAdDLManager.ITEM_ICONURL, this.iconUrl);
        contentValues.put("appVersionCode", Integer.valueOf(this.appVersionCode));
        contentValues.put(WfConstant.EVENT_KEY_TASK_ID, this.taskId);
    ...
    public void updateSpData() {
        Context contextOa;
        if (this.spData == null || (contextOa = com.ss.android.socialbase.downloader.downloader.fx.oa()) == null) {
            return;
        }
        com.bytedance.sdk.openadsdk.api.plugin.nr.nr(contextOa, "sp_download_info", 0).edit().putString(Integer.toString(getId()), this.spData.toString()).apply();
    }
    ...
    public DownloadInfo() {
        ...
        this.asyncHandleStatus = com.ss.android.socialbase.downloader.constants.u.ASYNC_HANDLE_NONE;
        ...
        this.isFirstDownload = true;
        this.isFirstSuccess = true;
        this.byteInvalidRetryStatus = com.ss.android.socialbase.downloader.constants.nr.BYTE_INVALID_RETRY_STATUS_NONE;
        this.enqueueType = EnqueueType.ENQUEUE_NONE;
    ...
    private DownloadInfo(u uVar) {
        ...
        String str = uVar.pn;
        this.tempPath = str;
        if (TextUtils.isEmpty(str) && !iz.n(strPn)) {
            this.tempPath = iz.iz();
        }
        safePutToDBJsonData("task_key", uVar.eh);
        if (uVar.v) {
            if (com.ss.android.socialbase.downloader.downloader.fx.kj().nr(getId()) == null) {
                this.savePath = iz.pn(this.savePath, this.url);
                this.tempPath = iz.pn(this.tempPath, this.url);
            }
        } else {
            com.ss.android.socialbase.downloader.fx.u.pn(TAG, "The distinct directory option is not set, which may cause 1005 problems and file downloads being covered");
        }
        this.status = new AtomicInteger(0);
        this.curBytes = new AtomicLong(0L);
        this.extra = uVar.x;
        this.onlyWifi = uVar.iz;
        this.extraHeaders = uVar.n;
        this.maxBytes = uVar.f10618a;
        this.retryCount = uVar.l;
        this.backUpUrlRetryCount = uVar.mv;
        this.force = uVar.s;
        this.outIp = uVar.jk;
        this.outSize = uVar.t;
        this.needPostProgress = uVar.k;
        this.maxProgressCount = uVar.my;
        this.minProgressTimeMsInterval = uVar.o;
        this.backUpUrls = uVar.sx;
        this.showNotification = uVar.bg;
        this.mimeType = uVar.dw;
        this.needHttpsToHttpRetry = uVar.c;
        this.needRetryDelay = uVar.rh;
        this.retryDelayTimeArray = uVar.ja;
        this.autoResumed = uVar.q;
        this.showNotificationForAutoResumed = uVar.qq;
        this.needDefaultHttpServiceBackUp = uVar.kj;
        this.needReuseChunkRunnable = uVar.z;
        this.packageName = uVar.gi;
        this.md5 = uVar.d;
        this.needReuseFirstConnection = uVar.bf;
        this.needIndependentProcess = uVar.wq;
        this.enqueueType = uVar.pb;
        this.headConnectionAvailable = uVar.xg;
        this.ignoreDataVerify = uVar.m;
        this.addListenerToSameTask = uVar.oa;
        this.needChunkDowngradeRetry = uVar.f10619jp;
        this.iconUrl = uVar.y;
        this.throttleNetSpeed = uVar.bc;
        this.openLimitSpeed = uVar.xw;
        JSONObject jSONObject = uVar.w;
        if (jSONObject != null) {
            safePutToDBJsonData("download_setting", jSONObject.toString());
        }
        safePutToDBJsonData("dbjson_key_expect_file_length", Long.valueOf(uVar.h));
        safePutToDBJsonData("executor_group", Integer.valueOf(uVar.su));
        safePutToDBJsonData("auto_install", Integer.valueOf(uVar.bq ? 1 : 0));
        this.needSDKMonitor = uVar.cj;
        this.monitorScene = uVar.tk;
        this.extraMonitorStatus = uVar.wi;
        this.expiredRedownload = uVar.mh;
        this.deleteCacheIfCheckFailed = uVar.yd;
        this.ttnetProtectTimeout = uVar.ay;
        this.distinctDirectory = uVar.v;
    ...
    public DownloadInfo(Cursor cursor) {
        ...
        try {
            ...
            if (columnIndex27 != -1) {
                ...
                x xVar2 = x.DELAY_RETRY_WAITING;
                if (i == xVar2.ordinal()) {
                    this.retryDelayStatus = xVar2;
                } else {
                    x xVar3 = x.DELAY_RETRY_DOWNLOADING;
                    if (i == xVar3.ordinal()) {
                        this.retryDelayStatus = xVar3;
                    } else {
                        x xVar4 = x.DELAY_RETRY_DOWNLOADED;
                        if (i == xVar4.ordinal()) {
                            this.retryDelayStatus = xVar4;
                        } else {
                            this.retryDelayStatus = xVar;
                        }
                    }
            ...
            int columnIndex38 = cursor.getColumnIndex(LxAdDLManager.ITEM_ICONURL);
            if (columnIndex38 != -1) {
            ...
            int columnIndex40 = cursor.getColumnIndex(WfConstant.EVENT_KEY_TASK_ID);
            if (columnIndex40 != -1) {
