...
public class ContainerBottomSheetBehavior<V extends View> extends CoordinatorLayout.Behavior<V> {
    ...
    */
    public final void setPeekHeight(int i) {
        ...
        } else if (this.Yl || this.Yk != i) {
            this.Yl = false;
            this.Yk = Math.max(0, i);
            this.Yq = this.Yz - i;
        }
        if (!z || this.mState != 4 || (weakReference = this.YA) == null || (v = weakReference.get()) == null) {
            return;
        }
        v.requestLayout();
    }
    ...
    public final void i(@FloatRange(from = 0.0d, fromInclusive = false, to = WeightedLatLng.DEFAULT_INTENSITY, toInclusive = false) float f) {
        this.YK = 0.8f;
    }
