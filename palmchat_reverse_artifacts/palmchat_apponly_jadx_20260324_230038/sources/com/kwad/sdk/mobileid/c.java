...
public final class c {
    ...
    private static void A(final com.kwad.sdk.commercial.c.a aVar) {
        GlobalThreadPools.Lq().execute(new bg() { // from class: com.kwad.sdk.mobileid.c.1
    ...
    private static UaidInfo a(Context context, int i, d dVar) {
        ...
        String strDF = bd.dF(context);
        return UaidInfo.newBuilder().et(i).gF(traceId).gE(dVar.Po()).gG(dVar.Pp()).eu(ao.e(context, strDF, zUseNetworkStateDisable)).ev(ao.dp(context)).bS((strDF == null || TextUtils.isEmpty(strDF.trim())) ? false : true).bQ(!zUseNetworkStateDisable).bR(!zUsePhoneStateDisable).Pq();
    }
}
