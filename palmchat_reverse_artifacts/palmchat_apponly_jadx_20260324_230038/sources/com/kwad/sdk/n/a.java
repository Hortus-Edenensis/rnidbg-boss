    ...
    public static void a(Activity activity, @NonNull List<d.a> list) {
        try {
            for (d.a aVar : list) {
                if (aVar != null) {
                    String strE = "";
                    if (TextUtils.equals(activity.getClass().getName(), aVar.bcd)) {
                        strE = aVar.bcd;
                    } else if (!TextUtils.isEmpty(aVar.bce) && activity.getClass().getName().startsWith(aVar.bcd)) {
                        strE = e(activity, aVar.bce);
                    }
