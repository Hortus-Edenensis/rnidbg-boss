...
public class AJVoice implements AudioTrack.OnPlaybackPositionUpdateListener {
    ...
    private long getPlaybackHeadPosition() {
        ...
        long playbackHeadPosition = ((long) this.mAudioTrack.getPlaybackHeadPosition()) & MuxerUtil.UNSIGNED_INT_MAX_VALUE;
    ...
    private int reconfigure() {
        ...
        if (minBufferSize <= 0) {
            k.nr(TAG, "getMinBufferSize failed, trace: sampleRate = " + this.mSampleRate + ", mChannelsLayout = " + this.mChannelsLayout + ", mAudioFormat = " + this.mAudioFormat);
        ...
        try {
            ...
            try {
                ...
                k.nr(TAG, "create audiotrack but failed to initialize");
                return -3;
            } catch (Throwable th) {
                k.nr(TAG, "create audio track failed ,detail = ".concat(String.valueOf(th)));
                return -1;
            }
        } catch (OutOfMemoryError unused2) {
            k.nr(TAG, "out of memory error when new audio buffer for audiotrack");
            return -10;
        }
    }
    ...
    public int start() {
        if (this.mAudioTrack == null) {
            k.nr(TAG, "audiotrack start before created");
            return -2;
        }
        this.mStoped = false;
        try {
            ...
            k.nr(TAG, "audioTrack start failed = ".concat(String.valueOf(th)));
            return -3;
        }
    }
    ...
    public int write(byte[] bArr, int i, int i2, int i3) {
        ...
        try {
            ...
            k.nr(TAG, "write failed : ret: " + iWrite + ", size = " + i2);
            return iWrite;
        } catch (Exception unused) {
            return -1;
        }
        ...
        if (bArr != null && bArr.length != 0) {
            ...
            if (i4 == -1) {
                ...
                if (Math.abs(writtenDurationMs - j) > 200) {
                    k.nr(TAG, "Discontinuity detected [expected " + writtenDurationMs + ", got " + j + "]");
