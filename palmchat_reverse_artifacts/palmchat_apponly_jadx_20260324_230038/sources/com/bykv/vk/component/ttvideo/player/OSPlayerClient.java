...
public class OSPlayerClient extends MediaPlayerClient {
    ...
    public void releaseAsync() {
        new fx(new Runnable() { // from class: com.bykv.vk.component.ttvideo.player.OSPlayerClient.1
    ...
    public void setPlaybackParams(PlaybackParams playbackParams) {
        if (Build.VERSION.SDK_INT >= 23) {
            android.media.PlaybackParams playbackParamsA = k21.a();
            float speed = playbackParams.getSpeed();
            if (speed > 0.0f) {
                playbackParamsA.setSpeed(speed);
            }
            int audioFallbackMode = playbackParams.getAudioFallbackMode();
            if (audioFallbackMode >= 0) {
                playbackParamsA.setAudioFallbackMode(audioFallbackMode);
            }
            float pitch = playbackParams.getPitch();
            if (pitch > 0.0f) {
                playbackParamsA.setPitch(pitch);
            }
            android.media.MediaPlayer mediaPlayer = this.mPlayer;
            if (mediaPlayer != null) {
                try {
                    mediaPlayer.setPlaybackParams(playbackParamsA);
                } catch (Exception unused) {
                }
            }
        }
