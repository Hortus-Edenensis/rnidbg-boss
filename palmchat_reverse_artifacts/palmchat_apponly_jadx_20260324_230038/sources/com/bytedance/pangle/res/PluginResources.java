...
public class PluginResources extends Resources {
    ...
    public static AssetManager appendHostRes(Resources resources) {
        String strU = x.u(Zeus.getAppApplication());
        String strNr = x.nr(Zeus.getAppApplication());
        List<String> listNr = t.nr();
        u uVar = new u();
        AssetManager assets = resources.getAssets();
        AssetManager assets2 = Zeus.getAppApplication().getAssets();
        HashSet hashSet = new HashSet(t.u(assets));
        List<String> listU = t.u(assets2);
        for (String str : listNr) {
            if (!hashSet.contains(str)) {
                assets = uVar.u(assets, str, true);
            }
        }
        for (String str2 : listU) {
            if (!isOtherPlugin(str2, strU, strNr) && !hashSet.contains(str2) && !listNr.contains(str2)) {
                assets = uVar.u(assets, str2, false);
            }
        }
        ZeusLogger.i(ZeusLogger.TAG_RESOURCES, "pluginAssets = " + t.nr(assets));
        return assets;
    }

    private Resources.NotFoundException handleException(Resources.NotFoundException notFoundException) {
        return new Resources.NotFoundException(("Resources#Assets: " + t.nr(getAssets())) + "," + notFoundException.getMessage());
    }
