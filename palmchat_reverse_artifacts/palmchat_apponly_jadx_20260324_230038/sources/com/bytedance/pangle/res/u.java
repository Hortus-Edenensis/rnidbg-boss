...
public class u {
    ...
    private LinkedHashMap<String, Integer> nr;

    static {
        List<String> listU = t.u();
    ...
    private AssetManager fx(AssetManager assetManager, String str, boolean z) {
        int iIntValue;
        int i = 3;
        Throwable th = null;
        int i2 = 3;
    ...
    private AssetManager nr(AssetManager assetManager, String str, boolean z) {
        String str2 = "addAssetPath";
        String str3 = z ? "addAssetPathAsSharedLibrary" : "addAssetPath";
        if (a.sx() && !z && str.startsWith("/product/overlay/")) {
            str3 = "addOverlayPath";
        }
    ...
    public AssetManager u(AssetManager assetManager, String str, boolean z) {
        AssetManager assetManagerU;
        if (str.endsWith(".frro")) {
            ZeusLogger.w(ZeusLogger.TAG_LOAD, "AssetManagerProcessor updateAssetManager skip frro. ".concat(str));
            return assetManager;
        }
        if (!a.u()) {
            assetManagerU = u(assetManager, str);
        } else if (a.x()) {
            assetManagerU = fx(assetManager, str, z);
            if (!t.u(assetManagerU, str)) {
                assetManagerU = nr(assetManager, str, z);
            }
        } else {
            assetManagerU = nr(assetManager, str, z);
        }
        ...
        return assetManagerU;
    }

    private AssetManager u(AssetManager assetManager, String str) {
        AssetManager assetManager2;
        List<String> listU = t.u(assetManager);
        ArrayList<String> arrayList = new ArrayList();
        StringBuilder sb = new StringBuilder();
        for (String str2 : listU) {
            if (!u.containsKey(str2) && !this.nr.containsKey(str2) && !str2.equals(str)) {
                arrayList.add(str2);
            }
        }
        ...
        try {
            ...
            synchronized (this.nr) {
                for (Map.Entry<String, Integer> entry : this.nr.entrySet()) {
                    if (!u.containsKey(entry.getKey())) {
                        sb.append(entry.getKey());
                        nr(assetManager2, entry.getKey(), false);
                    }
                }
            }
            if (!sb.toString().contains(Zeus.getAppApplication().getApplicationInfo().sourceDir)) {
                nr(assetManager2, Zeus.getAppApplication().getApplicationInfo().sourceDir, false);
                ZeusLogger.w(ZeusLogger.TAG_LOAD, "AssetManagerProcessor newAssetManager lost host path : " + u.containsKey(Zeus.getAppApplication().getApplicationInfo().sourceDir));
            }
            sb.append(str);
            nr(assetManager2, str, false);
            if (!arrayList.isEmpty()) {
                for (String str3 : arrayList) {
                    sb.append(str3);
                    nr(assetManager2, str3, false);
                }
            }
            if (a.iz() && !sb.toString().toLowerCase().contains("webview")) {
                try {
