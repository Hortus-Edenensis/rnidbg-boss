...
import com.bytedance.sdk.openadsdk.my.u.fx.nr;
...
public abstract class MediationCustomAdBaseLoader extends u {
    ...
    public <T> T applyFunction(int i, ValueSet valueSet, Class<T> cls) {
        if (i == 8241) {
            load((Context) valueSet.objectValue(AVMDLDataLoader.KeyIsLiveWaitP2pReadyThreshold, Context.class), wc7.k((SparseArray) valueSet.objectValue(8424, SparseArray.class)).a());
        } else if (i == 8225) {
            iz.fx(MediationConstant.TAG, "MediationCustomBaseLoader receiveBidResult");
            ...
            iz.fx(MediationConstant.TAG, "MediationCustomBaseLoader onPause");
            onPause();
        } else if (i == 8148) {
            iz.fx(MediationConstant.TAG, "MediationCustomBaseLoader onPause");
            onResume();
        } else if (i == 8109) {
            iz.fx(MediationConstant.TAG, "MediationCustomBaseLoader onDestroy");
            onDestroy();
        }
    ...
    public final void load(Context context, ValueSet valueSet) {
        ...
        loadInner(context, nr.u(adSlotValueSet != null ? adSlotValueSet.sparseArray() : new SparseArray<>()), new com.bytedance.sdk.openadsdk.mediation.custom.MediationCustomServiceConfig(mediationLoaderConfigCreate.getMediationCustomServiceConfigValue()), mediationLoaderConfigCreate.getGMCustomAdLoader());
    }
    ...
    public Integer u(MediationConstant.AdIsReadyStatus adIsReadyStatus) {
