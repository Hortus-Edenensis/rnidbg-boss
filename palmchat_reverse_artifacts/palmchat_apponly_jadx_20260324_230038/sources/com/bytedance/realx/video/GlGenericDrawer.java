    ...
    public static String createFragmentShaderString(String str, ShaderType shaderType, FilterType filterType) {
        ...
        if (shaderType == ShaderType.YUV) {
            ...
            if (filterType == FilterType.Bilinear) {
                ...
                sb.append("  vec2 c_onePixel =  1.0 / ori_tex_size;\n");
                sb.append("  float C11, C12, C13, C21, C22, C23, C31, C32, C33;\n");
                sb.append("  C11 = texture2D(tex, p - c_onePixel).r;\n");
                sb.append("  C12 = texture2D(tex, p + vec2( 0, -c_onePixel.y)).r;\n");
                sb.append("  C13 = texture2D(tex, p + vec2( c_onePixel.x, -c_onePixel.y)).r;\n");
                sb.append("  C21 = texture2D(tex, p + vec2( -c_onePixel.x , 0)).r;\n");
                sb.append("  C22 = texture2D(tex, p).r;\n");
                sb.append("  C23 = texture2D(tex, p + vec2( c_onePixel.x , 0)).r;\n");
                sb.append("  C31 = texture2D(tex, p + vec2( -c_onePixel.x , c_onePixel.y)).r;\n");
                sb.append("  C32 = texture2D(tex, p + vec2( 0 , c_onePixel.y)).r;\n");
                sb.append("  C33 = texture2D(tex, p + c_onePixel).r;\n");
                sb.append("  return (C11 + C12 + C13 + C21 + C22 + C23 + C31 + C32 + C33) / 9.0;\n");
                sb.append("}\n");
                sb.append("vec4 sample(vec2 p) {\n");
                sb.append("  float y = box_filter(y_tex, p) * 1.16438;\n");
                sb.append("  float u = box_filter(u_tex, p);\n");
                sb.append("  float v = box_filter(v_tex, p);\n");
                sb.append("  return vec4(y + 1.59603 * v - 0.874202,\n");
                sb.append("    y - 0.391762 * u - 0.812968 * v + 0.531668,\n");
                sb.append("    y + 2.01723 * u - 1.08563, 1);\n");
                sb.append("}\n");
            ...
            if (filterType == FilterType.Bilinear) {
                ...
                sb.append("  vec2 c_onePixel =  1.0 / ori_tex_size;\n");
                sb.append("  vec3 C11 = texture2D(tex, p - c_onePixel).rgb;\n");
                sb.append("  vec3 C12 = texture2D(tex, p + vec2( 0, -c_onePixel.y)).rgb;\n");
                sb.append("  vec3 C13 = texture2D(tex, p + vec2( c_onePixel.x, -c_onePixel.y)).rgb;\n");
                sb.append("  vec3 C21 = texture2D(tex, p + vec2( -c_onePixel.x , 0)).rgb;\n");
                sb.append("  vec3 C22 = texture2D(tex, p).rgb;\n");
                sb.append("  vec3 C23 = texture2D(tex, p + vec2( c_onePixel.x , 0)).rgb;\n");
                sb.append("  vec3 C31 = texture2D(tex, p + vec2( -c_onePixel.x , c_onePixel.y)).rgb;\n");
                sb.append("  vec3 C32 = texture2D(tex, p + vec2( 0 , c_onePixel.y)).rgb;\n");
                sb.append("  vec3 C33 = texture2D(tex, p + c_onePixel).rgb;\n");
                sb.append("  return vec4((C11 + C12 + C13 + C21 + C22 + C23 + C31 + C32 + C33) / 9.0, 1.0);\n");
                sb.append("}\n");
                sb.append(str);
            } else {
