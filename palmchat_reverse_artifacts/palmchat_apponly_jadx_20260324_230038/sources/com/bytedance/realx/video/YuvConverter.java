...
public class YuvConverter {
        ...
        if (list != null) {
            if (list.size() == 0) {
                ...
                while (true) {
                    ...
                    int iIntValue = this.mTextureFreeQueue.get(i3).intValue();
                    if (this.mTextureWidthMap.get(Integer.valueOf(iIntValue)).intValue() == i && this.mTextureHeightMap.get(Integer.valueOf(iIntValue)).intValue() == i2) {
                        break;
                    }
                    i3++;
                }
    ...
    public VideoFrame.I420Buffer convert(VideoFrame.TextureBuffer textureBuffer, FilterType filterType, int i, int i2) {
        ...
        if (iGlGetError == 0) {
            ...
            return JavaI420Buffer.wrap(width, height, byteBufferSlice, i3, byteBufferSlice2, i3, byteBufferNativeAllocateByteBuffer.slice(), i3, new Runnable() { // from class: op6
