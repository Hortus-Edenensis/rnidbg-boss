...
public class OverlayInterpolationShaderProgram extends FrameCacheGlShaderProgram {
    ...
    public OverlayInterpolationShaderProgram(Context context, boolean z, @FloatRange(from = 0.0d) float f, float f2) throws VideoFrameProcessingException {
        super(context, 1, z);
        this.wrappedGlShaderProgram = new OverlayBlendTwoTextureShaderProgram(context, z, f, false);
        this.useHdr = z;
    ...
    private void queueAlphaBlendingFrame(GlObjectsProvider glObjectsProvider, GlTextureInfo glTextureInfo, long j) {
        try {
            ...
            GlUtil.clearFocusedBuffers();
            this.wrappedGlShaderProgram.drawFrame(new InputFrameInfoPair(new InputFrameInfo(glTextureInfo2, j), new InputFrameInfo(glTextureInfo, j)));
