...
import com.oplus.tbl.exoplayer2.database.DatabaseIOException;
import com.oplus.tbl.exoplayer2.database.DatabaseProvider;
import com.oplus.tbl.exoplayer2.database.VersionTable;
...
public final class DefaultDownloadIndex implements WritableDownloadIndex {
    ...
    public static final class DownloadCursorImpl implements DownloadCursor {
        ...
        @Override // com.oplus.tbl.exoplayer2.offline.DownloadCursor, java.io.Closeable, java.lang.AutoCloseable
        public void close() {
            this.cursor.close();
        }

        @Override // com.oplus.tbl.exoplayer2.offline.DownloadCursor
        public int getCount() {
    ...
    private static Download getDownloadForCurrentRowV2(Cursor cursor) {
        DownloadRequest downloadRequestBuild = new DownloadRequest.Builder(cursor.getString(0), Uri.parse(cursor.getString(2))).setMimeType(inferMimeType(cursor.getString(1))).setStreamKeys(decodeStreamKeys(cursor.getString(3))).setCustomCacheKey(cursor.getString(4)).setData(cursor.getBlob(5)).build();
        DownloadProgress downloadProgress = new DownloadProgress();
        downloadProgress.bytesDownloaded = cursor.getLong(13);
        downloadProgress.percentDownloaded = cursor.getFloat(12);
        int i = cursor.getInt(6);
    ...
    private static String inferMimeType(String str) {
        return "dash".equals(str) ? "application/dash+xml" : LiveConfigKey.HLS.equals(str) ? "application/x-mpegURL" : "ss".equals(str) ? "application/vnd.ms-sstr+xml" : "video/x-unknown";
    }
    ...
    @Override // com.oplus.tbl.exoplayer2.offline.DownloadIndex
    @Nullable
    ...
    @Override // com.oplus.tbl.exoplayer2.offline.WritableDownloadIndex
