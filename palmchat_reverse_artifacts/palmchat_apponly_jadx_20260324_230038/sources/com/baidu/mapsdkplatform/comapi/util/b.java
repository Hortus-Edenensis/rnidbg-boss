...
public class b {
    ...
    static double[] f4004a = {1.289059486E7d, 8362377.87d, 5591021.0d, 3481989.83d, 1678043.12d, 0.0d};
    ...
    public static a a(a aVar, double[] dArr) {
        a aVar2 = new a();
        aVar2.f4005a = dArr[0] + (dArr[1] * Math.abs(aVar.f4005a));
        double dAbs = Math.abs(aVar.b) / dArr[9];
        double d2 = dArr[2] + (dArr[3] * dAbs) + (dArr[4] * dAbs * dAbs) + (dArr[5] * dAbs * dAbs * dAbs) + (dArr[6] * dAbs * dAbs * dAbs * dAbs) + (dArr[7] * dAbs * dAbs * dAbs * dAbs * dAbs) + (dArr[8] * dAbs * dAbs * dAbs * dAbs * dAbs * dAbs);
        aVar2.b = d2;
        aVar2.f4005a *= (double) (aVar.f4005a < 0.0d ? -1 : 1);
        aVar2.b = d2 * ((double) (aVar.b < 0.0d ? -1 : 1));
        return aVar2;
    }
    ...
    public static List<List<LatLng>> c(String str) {
        ...
        ComplexPt complexPtA = com.baidu.mapsdkplatform.comjni.tools.a.a(str);
        if (complexPtA == null || (arrayList = complexPtA.mGeoPt) == null) {
            return null;
        }
        ...
        for (ArrayList<Point> arrayList3 : arrayList) {
            ...
            for (Point point : arrayList3) {
                if (SDKInitializer.getCoordType() == CoordType.GCJ02) {
                    arrayList4.add(CoordTrans.baiduToGcj(a(new GeoPoint(point.getDoubleY() / 100.0d, point.getDoubleX() / 100.0d))));
                } else {
                    arrayList4.add(a(new GeoPoint(point.getDoubleY() / 100.0d, point.getDoubleX() / 100.0d)));
                }
            }
    ...
    public static LatLng a(GeoPoint geoPoint) {
        ...
        a aVar = new a();
        aVar.f4005a = geoPoint.getLongitudeE6();
        aVar.b = geoPoint.getLatitudeE6();
        a aVar2 = new a();
        double d2 = aVar.f4005a;
        aVar2.f4005a = d2;
        if (d2 > 2.0037508342E7d) {
            aVar2.f4005a = (d2 - 2.0037508342E7d) - 2.0037508342E7d;
        } else if (d2 < -2.0037508342E7d) {
            aVar2.f4005a = 2.0037508342E7d - ((-2.0037508342E7d) - d2);
        }
        double d3 = aVar.b;
        aVar2.b = d3;
        if (d3 < 1.0E-6d && d3 >= 0.0d) {
            aVar2.b = 1.0E-6d;
        } else if (d3 < 0.0d && d3 > -1.0E-6d) {
            aVar2.b = -1.0E-6d;
        ...
        while (true) {
            ...
            if (Math.abs(aVar2.b) > f4004a[i]) {
                dArr = c[i];
                break;
            }
            i++;
        }
        a aVarA = a(aVar2, dArr);
        return new LatLng(aVarA.b, aVarA.f4005a);
    }

    public static GeoPoint a(LatLng latLng) {
        a aVar = new a();
        double[] dArr = new double[10];
        double dAbs = Math.abs(latLng.latitude * 1000000.0d);
        aVar.b = dAbs;
        if (dAbs < 0.1d) {
            aVar.b = 0.1d;
        }
        ...
        aVar.f4005a = latLng.longitude;
        aVar.b = latLng.latitude;
        a aVarA = a(aVar, dArr);
        return new GeoPoint(aVarA.b, aVarA.f4005a);
    }
