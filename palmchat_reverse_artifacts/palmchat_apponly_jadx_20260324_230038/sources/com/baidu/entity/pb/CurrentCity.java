...
public final class CurrentCity extends MessageMicro {
    ...
    public int getSerializedSize() {
        ...
        int iComputeInt32Size = hasCode() ? CodedOutputStreamMicro.computeInt32Size(1, getCode()) + 0 : 0;
        if (hasGeo()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeStringSize(2, getGeo());
        }
        if (hasLevel()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeInt32Size(3, getLevel());
        }
        if (hasName()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeStringSize(4, getName());
        }
        if (hasSupSubway()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeBoolSize(5, getSupSubway());
        }
        if (hasSupLukuang()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeBoolSize(6, getSupLukuang());
        }
        if (hasUid()) {
            iComputeInt32Size += CodedOutputStreamMicro.computeStringSize(7, getUid());
        }
        ...
        int size = iComputeInt32Size + iComputeSInt32SizeNoTag + (getSgeoList().size() * 1);
