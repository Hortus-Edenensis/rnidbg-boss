...
public final class end_button_info extends MessageMicro {
    ...
    public int getSerializedSize() {
        int iComputeBytesSize = hasBtnTitle() ? 0 + CodedOutputStreamMicro.computeBytesSize(1, getBtnTitle()) : 0;
        if (hasEndInput()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeBytesSize(2, getEndInput());
        }
        if (hasShowPoint()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeMessageSize(3, getShowPoint());
        }
        if (hasUid()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeBytesSize(4, getUid());
        }
        if (hasCname()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeBytesSize(5, getCname());
        }
        if (hasCityid()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeInt32Size(6, getCityid());
        }
        if (hasBorderColor()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeInt32Size(7, getBorderColor());
        }
        if (hasBackgroundColor()) {
            iComputeBytesSize += CodedOutputStreamMicro.computeInt32Size(8, getBackgroundColor());
        }
