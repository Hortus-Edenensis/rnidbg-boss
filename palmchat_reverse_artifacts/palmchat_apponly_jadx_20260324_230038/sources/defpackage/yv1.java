package defpackage;

import com.zenmen.find.ConditionHelper;
import org.json.JSONException;
import org.json.JSONObject;

/* JADX INFO: compiled from: SearchBox */
/* JADX INFO: loaded from: classes12.dex */
public class yv1 extends gu3 {
    public yv1(int i, String str) {
        super(i, str);
    }

    public final JSONObject W(boolean z) {
        JSONObject jSONObjectO = O(z);
        try {
            if (z) {
                jSONObjectO.put("version", 0);
            } else {
                jSONObjectO.put("version", this.w);
            }
            boolean zM = ap3.a().M();
            if (zM) {
                jSONObjectO.put("hitLx68239", zM);
            }
        } catch (JSONException e) {
            e.printStackTrace();
        }
        return ConditionHelper.getInstance().getRecommendCond().mergeParams(jSONObjectO);
    }

    @Override // defpackage.gu3, defpackage.om2
    public JSONObject a() {
        return W(false);
    }

    @Override // defpackage.gu3, defpackage.om2
    public JSONObject b() {
        return W(true);
    }

    @Override // defpackage.gu3, defpackage.br
    public boolean t() {
        return true;
    }

    @Override // defpackage.gu3, defpackage.br
    public boolean u() {
        return false;
    }
}
