    ...
    */
    public InlineClassAwareCaller(CallableMemberDescriptor descriptor, Caller<? extends M> caller, boolean z) {
        ...
        if (InlineClassesUtilsKt.isGetterOfUnderlyingPropertyOfInlineClass(descriptor)) {
            ...
            int size = arrayList.size() + i + (this.isDefault ? (((arrayList.size() + 32) - 1) / 32) + 1 : 0) + (((descriptor instanceof FunctionDescriptor) && ((FunctionDescriptor) descriptor).isSuspend()) ? 1 : 0);
            ...
            IntRange intRangeUntil = RangesKt___RangesKt.until(Math.max(i, 0), arrayList.size() + i);
            Method[] methodArr = new Method[size];
            int i2 = 0;
            while (i2 < size) {
                methodArr[i2] = (!(i2 <= intRangeUntil.getLast() && intRangeUntil.getFirst() <= i2) || (inlineClass = InlineClassAwareCallerKt.toInlineClass((KotlinType) arrayList.get(i2 - i))) == null) ? null : InlineClassAwareCallerKt.getUnboxMethod(inlineClass, descriptor);
                i2++;
            }
    ...
    public M mo2135getMember() {
        return (M) this.caller.mo2135getMember();
    }
