...
public class TypeUtils {
    ...
    */
    private static /* synthetic */ void $$$reportNull$$$0(int i) {
        ...
        switch (i) {
            ...
            case 16:
                ...
                break;
            case 20:
                objArr[0] = SharePluginInfo.ISSUE_SUB_TYPE;
                break;
            case 21:
                objArr[0] = "superType";
                break;
    ...
    public static List<KotlinType> getImmediateSupertypes(KotlinType kotlinType) {
        ...
        Collection<KotlinType> collectionMo2144getSupertypes = kotlinType.getConstructor().mo2144getSupertypes();
        ArrayList arrayList = new ArrayList(collectionMo2144getSupertypes.size());
        Iterator<KotlinType> it = collectionMo2144getSupertypes.iterator();
        while (it.hasNext()) {
    ...
    public static TypeParameterDescriptor getTypeParameterDescriptorOrNull(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2143getDeclarationDescriptor() instanceof TypeParameterDescriptor) {
            return (TypeParameterDescriptor) kotlinType.getConstructor().mo2143getDeclarationDescriptor();
        }
        return null;
    }
    ...
    public static boolean hasNullableSuperType(KotlinType kotlinType) {
        ...
        if (kotlinType.getConstructor().mo2143getDeclarationDescriptor() instanceof ClassDescriptor) {
            return false;
        }
    ...
    public static boolean isNullableType(KotlinType kotlinType) {
        ...
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it = constructor.mo2144getSupertypes().iterator();
            while (it.hasNext()) {
                if (isNullableType(it.next())) {
                    return true;
                }
    ...
    private static boolean contains(KotlinType kotlinType, Function1<UnwrappedType, Boolean> function1, SmartSet<KotlinType> smartSet) {
        ...
        TypeConstructor constructor = kotlinType.getConstructor();
        if (constructor instanceof IntersectionTypeConstructor) {
            Iterator<KotlinType> it = ((IntersectionTypeConstructor) constructor).mo2144getSupertypes().iterator();
            while (it.hasNext()) {
