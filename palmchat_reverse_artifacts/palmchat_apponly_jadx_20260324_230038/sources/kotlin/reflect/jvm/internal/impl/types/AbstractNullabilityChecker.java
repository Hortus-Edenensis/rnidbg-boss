            ...
            while (!supertypesDeque.isEmpty()) {
                ...
                if (supertypesSet.add(current)) {
                    ...
                    if (supertypesPolicy2 == null) {
                        ...
                        while (it.hasNext()) {
                            SimpleTypeMarker simpleTypeMarkerMo2147transformType = supertypesPolicy2.mo2147transformType(typeCheckerState, it.next());
                            if ((typeSystemContext.isClassType(simpleTypeMarkerMo2147transformType) && !typeSystemContext.isMarkedNullable(simpleTypeMarkerMo2147transformType)) || typeSystemContext.isDefinitelyNotNullType(simpleTypeMarkerMo2147transformType)) {
                                typeCheckerState.clear();
                            } else {
                                supertypesDeque.add(simpleTypeMarkerMo2147transformType);
                            }
                        }
                    }
                }
        ...
        while (!supertypesDeque.isEmpty()) {
            ...
            if (supertypesSet.add(current)) {
                ...
                if (supertypesPolicy == null) {
                    ...
                    Iterator<KotlinTypeMarker> it = typeSystemContext2.supertypes(typeSystemContext2.typeConstructor(current)).iterator();
                    while (it.hasNext()) {
                        SimpleTypeMarker simpleTypeMarkerMo2147transformType = supertypesPolicy.mo2147transformType(state, it.next());
                        if (INSTANCE.isApplicableAsEndNode(state, simpleTypeMarkerMo2147transformType, end)) {
                            state.clear();
                            return true;
                        }
                        supertypesDeque.add(simpleTypeMarkerMo2147transformType);
